...
void manejar_solicitud(int client_fd) {
    ...
    if (tamano_lectura < 0) {
        perror("Error en la recepción de la solicitud");
        close(client_fd);
        return;
    }
    ...
    if (strcmp(solicitud, "estado") == 0) {
        ...
        if (placa >= 0 && placa < NUM_AUTOS) {
            ...
            snprintf(respuesta, sizeof(respuesta), "Placa inválida");
        }
    } else {
        snprintf(respuesta, sizeof(respuesta), "Comando no reconocido");
    }
...
// Función del hilo para manejar el cliente
void* manejar_cliente(void* params) {
    hilo_params_t* args = (hilo_params_t*) params;
    manejar_solicitud(args->client_fd);
    free(args);
    pthread_exit(NULL);
}
...
int main() {
    ...
    while (1) {
        ...
        pthread_t thread_id;
        hilo_params_t* params = malloc(sizeof(hilo_params_t));
        params->client_fd = client_fd;
        pthread_create(&thread_id, NULL, manejar_cliente, (void*)params);
        pthread_detach(thread_id);  // Hilo independiente
    }
